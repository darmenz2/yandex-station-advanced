...
class HLSCapture:
    def __init__(self, directory: Path, device_index: int | None = None, segment_time: float = 1.0,
        ...
        self.block_ms = 10 if self.latency_profile == 'fast' else 20
        self.start_buffer_seconds = .02 if self.latency_profile == 'fast' else .06
        ...
        self.max_pcm_age = 0.06 if self.latency_profile == 'fast' else 0.20
        ...
        self.frames = self.rate * self.block_ms // 1000
        self.p = self.stream = self.process = None
        self.stop_event = threading.Event()
        self.queue: queue.Queue[tuple[float, bytes]] = queue.Queue(maxsize=8)
    ...
    def _start(self) -> None:
        ...
        try:
            if not self.synthetic:
                ...
                self.frames = max(128, self.rate * self.block_ms // 1000)
                if self.transport == 'pcm' and self.channels not in (1, 2):
                    raise BridgeError('Прямой PCM требует моно или стерео. Этот выход многоканальный; выберите MP3 или стереовыход.')
                    ...
                    try:
                        ...
                        if in_data:
                            ...
                            with contextlib.suppress(queue.Full):
                                self.queue.put_nowait((time.monotonic(), in_data))
                        if status:
                            self.dropped += 1
                    except Exception:
            ...
            if self.transport == 'pcm':
                self.pcm_ring = FrameRing(64, frame_bytes=self.frames * self.channels * 2,
    ...
    def _write_pcm(self):
        with audio_thread_scope(self.latency_profile == 'fast') as scheduling:
        ...
        try:
            while not self.stop_event.is_set():
                ...
                if time.monotonic() - next_tick > (.06 if self.latency_profile == 'fast' else .3):
                    ...
                    self.stale_frames += len(buffer) // (self.channels * 2)
                    buffer.clear()
                if self.synthetic and (self.synthetic_duration is None or phase / self.rate < self.synthetic_duration):
                    pcm = array.array('h')
