...
class FrameRing:
    ...
    def append_packet(self, data: bytes) -> None:
        """Publish already available PCM units atomically; do not wait to batch.

        One driver callback can contain several units. Publishing the packet
        under one lock avoids waking a consumer on every split 1 ms unit.
        The time and memory bounds are the same as append().
        """
        if not data or len(data) % self.frame_bytes:
            raise ValueError('Incomplete audio packet')
        count = len(data) // self.frame_bytes
        # A corrupt/huge producer packet must not allocate an unbounded list.
        kept = min(count, self.frames.maxlen)
        offset = (count - kept) * self.frame_bytes
        units = tuple(data[i:i+self.frame_bytes]
                      for i in range(offset, len(data), self.frame_bytes))
        with self.lock:
            now = time.monotonic()
            if self.first_frame_at is None:
                self.first_frame_at = now
            self.last_frame_at = now
            self.frames.extend(units)
            self.total += count
    ...
    def read(self, cursor: int, count: int = 3) -> FrameRead:
        with self.lock:
            ...
            frames = tuple(islice(self.frames, cursor - first, cursor - first + count))
