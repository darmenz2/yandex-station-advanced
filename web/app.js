...
let activeModeSignature = ''; // restore only an already-running stream on page refresh 
...
function updateDisabled() {
  ...
  $('audioMode').disabled = active || state?.media?.kind === 'live';
  $('returnMain').disabled = active;
  ...
  $('autoLive').disabled = active;
  $('refreshInterval').disabled = active;
...
function render(next) {
  ...
  const continuity = state.continuity || {}, maintenance = continuity.maintenance || {};
  if (document.activeElement !== $('autoLive')) $('autoLive').checked = continuity.enabled !== false;
  if (document.activeElement !== $('refreshInterval')) $('refreshInterval').value = String(maintenance.interval_seconds ?? 240);
  const purpose = continuity.suspension_purpose === 'local_video' ? 'компенсация видео' : 'акустический замер';
  write('continuityStatus', continuity.calibrating ? `Автосброс временно отключён: ${purpose}.` : continuity.blocked_by_other_source ? 'Станция переключена на другой источник. Автосброс остановлен до ручного запуска.' : media.kind === 'live' ?
    `Плановых сбросов: ${maintenance.periodic_count || 0}; после тишины: ${continuity.triggers || 0}; восстановлений HTTP: ${maintenance.recovery_count || 0}. ` +
    (maintenance.interval_seconds ? `До плановой проверки: ${maintenance.next_periodic_in_seconds ?? '—'} с. ` : 'Плановые сбросы выключены. ') +
    `Дополнительный запас PCM: ${media.micro_buffer_effective_ms || 0} мс.` : 'Защита включится вместе с трансляцией.');
  if (media.kind === 'live' && capture) {
    const signature = `${media.transport}:${capture.latency_profile}`;
    if (signature !== activeModeSignature) {
      activeModeSignature = signature;
      $('audioMode').value = capture.latency_profile === 'experimental' ? 'experimental'
        : media.transport === 'pcm' && capture.latency_profile === 'fast' ? 'main' : 'fallback';
      if ($('audioMode').value === 'fallback') {
        $('transport').value = media.transport;
        $('latencyProfile').value = capture.latency_profile === 'fast' ? 'fast' : 'balanced';
      }
      updateTransportHint();
    }
  } else { activeModeSignature = ''; }
  if (initial) {
  ...
  if (capture) {
    ...
    if (capture.latency_profile) timing += ` · профиль: ${{fast:'блок 10 мс',balanced:'устойчивый',experimental:'экспериментальный / по готовности'}[capture.latency_profile] || capture.latency_profile} · блок: ${capture.requested_block_ms} мс`;
    if (capture.experimental_timing) {
      const et = capture.experimental_timing;
      timing += ` · стартовый запас: 0 мс`;
      if (et.callback_packet_ms?.p50 != null) timing += ` · реальный пакет Windows: ${et.callback_packet_ms.p50} мс (медиана)`;
      if (et.callback_interval_ms?.p95 != null) timing += ` · интервал callback P95: ${et.callback_interval_ms.p95} мс`;
      if (et.callback_to_ring_ms?.p95 != null) timing += ` · callback → выдача P95: ${et.callback_to_ring_ms.p95} мс`;
    }
  ...
  const quiet = capture && ['silent','no_frames'].includes(capture.signal_status) && capture.running;
  show('captureWarning', !!capture?.warning && !quiet);
  write('captureWarning', capture?.warning || '');
  if(quiet) write('captureDevice', `Тишина на выходе «${capture.device?.name || 'Windows'}» — поток поддерживается, ждём звук.`);
...
click('streamTest', () => command('stream_test', {...selectedLiveSettings(), segment_time:Number($('segment').value)}, 'Готовим тест выбранного режима без захвата Windows…'));
...
click('live', () => command('live', {...selectedLiveSettings(), device_index:$('outputs').value, device_key:$('outputs').selectedOptions[0]?.dataset.key || '', segment_time:Number($('segment').value)}, 'Захват включается. Готовим буфер трансляции…'));
...
function selectedLiveSettings() {
  const mode = $('audioMode').value;
  if (mode === 'main') return {playback_mode:'main', transport:'pcm', latency_profile:'fast'};
  if (mode === 'experimental') return {playback_mode:'experimental', transport:'pcm', latency_profile:'experimental'};
  return {playback_mode:'fallback', transport:$('transport').value,
    latency_profile:$('transport').value === 'hls' ? 'balanced' : $('latencyProfile').value};
}
function updateTransportHint() {
  const mode = $('audioMode').value;
  const fallback = mode === 'fallback';
  show('fallbackSettings', fallback);
  show('experimentalNotice', mode === 'experimental');
  show('returnMain', mode !== 'main');
  show('hlsBuffer', fallback && $('transport').value === 'hls');
  show('hlsTestDetails', fallback);
  write('modeHint', {
    main:'Основной: проверенный тракт 1.0.6. WAV/PCM, блок 10 мс, стартовый запас 20 мс; без кодировщика и изменения частоты. Настройки сети и разрешения не меняются.',
    experimental:'PCM Direct: запрос блока 1 мс; нулевой стартовый запас; отправка по готовности данных, без ожидания очередного такта. Устаревшие данные отбрасываются. Это может быть менее устойчиво, чем основной режим.',
    fallback:'Дополнительные способы передачи. Они не включаются автоматически при проблемах с экспериментом. Здесь можно вернуться к MP3, HLS или PCM с большим запасом.'
  }[mode]);
  write('transportHint', {
    pcm:'WAV/PCM без кодировщика. В устойчивом профиле блок 20 мс и стартовый запас 60 мс.',
    mp3:'Прямой MP3: 320 кбит/с, 48 кГц. Запас зависит от выбранного профиля.',
    hls:'HLS: сегменты 1 или 2 секунды; резерв совместимости с большей задержкой.'
  }[$('transport').value]);
  updateDisabled();
}
// Old MP3/HLS localStorage preferences must not override the new main mode.
// Remember only fallback details. Experiment always needs an explicit choice
// after a fresh idle page load; refreshing an ACTIVE stream mirrors its mode.
$('audioMode').value = 'main';
try {
...
$('audioMode').addEventListener('change', updateTransportHint);
...
click('returnMain', async () => {
  if (state?.media?.active) await command('stop', {}, 'Останавливаем текущий поток…');
  $('audioMode').value = 'main';
  activeModeSignature = '';
  updateTransportHint();
  notify('Выбран основной WAV/PCM, блок 10 мс. Нажмите «Транслировать звук ПК».');
});
$('autoLive').addEventListener('change', () => command('continuity', {enabled:$('autoLive').checked}).catch(()=>{}));
$('refreshInterval').addEventListener('change', () => command('maintenance', {interval_seconds:Number($('refreshInterval').value)}).catch(()=>{}));
$('calibrationLink').href = '/calibration.html#key='+encodeURIComponent(key);
$('playerLink').href = '/local-player.html#key='+encodeURIComponent(key);
updateTransportHint();
